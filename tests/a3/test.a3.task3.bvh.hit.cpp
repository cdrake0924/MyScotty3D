...
Test test_a3_task3_bvh_hit_simple_triangle_miss("a3.task3.bvh.hit.simple.triangle.miss", []() {
	std::vector<Indexed_Mesh::Vert> verts;
	verts.push_back({Vec3(0, 0, 0), Vec3(1, 0, 0), Vec2(0, 0), 0});
	verts.push_back({Vec3(1, 0, 0), Vec3(1, 0, 0), Vec2(0, 0), 1});
	verts.push_back({Vec3(0, 1, 0), Vec3(1, 0, 0), Vec2(0, 0), 2});
	...
	PT::Trace ret = mesh.hit(ray);
	if (ret.hit) {
		throw Test::error("BVH triangle hit reported a hit when it should miss.");
	}
});

Test test_a3_task3_bvh_hit_simple_sphere_inside("a3.task3.bvh.hit.simple.sphere.inside", []() {
	PT::Tri_Mesh mesh = PT::Tri_Mesh(Util::closed_sphere_mesh(1.0f, 1), true);
	Ray ray(Vec3(0, 0, 0), Vec3(0, 0, 1));
	PT::Trace ret = mesh.hit(ray);
	if (!ret.hit) {
		throw Test::error("BVH sphere hit missed when ray starts inside.");
	}
});
